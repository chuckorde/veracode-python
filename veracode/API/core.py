...
class REST(object):
    ...
    def __veracode_hmac(self, host, url, method):
        ...
        signature_key = hmac.new(key_date, 'vcode_request_version_1'.encode(), sha256).digest()
        signature = hmac.new(signature_key, signing_data.encode(), sha256).hexdigest()
