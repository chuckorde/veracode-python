...
from veracode import SDK, sandbox
...
class Application(object):
    def __new__(self, app_name=None):
        if app_name:
            return ExistingApplication(app_name)
        else:
...
class ExistingApplication(object):
    def __init__(self, app_name, sandbox_name=None):
    ...
    def _get_sandbox_by_name(self, sandbox):
    ...
    def flaws(self):
        if not self._flaws:
            self._flaws = _find(self.details.severity, 'flaw',
                    ['category', 'cwe', 'staticflaws'])
        return self._flaws
    ...
    def sandbox(self, obj):
        ...
        if isinstance(obj, sandbox.NewSandbox):
            self._sandbox = obj
        # create sandbox!
...
#         if not self._build:
#             if self.has_builds:
#                 self._build = self._builds.pop()
#         return self._build
#
#     @build.setter
#     def build(self, build):
#         # TODO: allow build by name
#         self._build = build
#
#     @property
#     def builds(self):
